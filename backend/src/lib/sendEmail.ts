...
export const sendInvoiceEmail = async (invoice: Invoice, pdfPath: string) => {
  const mailOptions = {
    ...
    html: `
            ...
            } is attached. The due date is ${new Date(
      invoice.dueDate
  ...
  try {
    ...
    console.log(
      `Email sent to ${invoice.client.email} with invoice #${invoice.invoiceNumber}`
    );
  } catch (error) {
...
export const sendRemainder = async (
  ...
  try {
    await transporter.sendMail({
      ...
      text: `Hi ${recipientName},\n\nThis is a reminder about your invoice status.\n\nBest regards,\nInvoiceGen Team`,
    });
    console.log("Email send to ", recipientEmail);
  } catch (error){
    console.error("Failed to send email:", error);
    throw new Error("Failed to send the invoice email.");
  }
